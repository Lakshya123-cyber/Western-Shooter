	...
	def input(self):
		...
		if not self.attacking:
			...
			if keys[pygame.K_SPACE]:
				...
				if self.status.split('_')[0] == 'left': self.bullet_direction = v2(-1, 0)
				if self.status.split('_')[0] == 'right': self.bullet_direction = v2(1, 0)
				if self.status.split('_')[0] == 'up': self.bullet_direction = v2(0, -1)
				if self.status.split('_')[0] == 'down': self.bullet_direction = v2(0, 1)
